...
function calculateInterest(uint basisPoints, uint weiAmont) private pure returns(uint) {
    return basisPoints * weiAmont / 10000;
}
...
function closePosition(uint positionId) external {
    require(positions[positionId].walletAddress == msg.sender, "Only position ceator may mdify position");
